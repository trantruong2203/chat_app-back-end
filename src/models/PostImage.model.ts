...
export const getPostImages = async (postid: number): Promise<PostImage[]> => {
    const [results] = await db.query('SELECT id, postid, imgurl FROM postimage WHERE postid = ?', [postid]);
    return results as PostImage[];
};

export const createPostImage = async (postid: number, imgurl: string): Promise<PostImage[]> => {
    const [results] = await db.query('INSERT INTO postimage (postid, imgurl) VALUES (?, ?) ', [postid, imgurl]);
    return results as PostImage[];
};
...
export const updatePostImage = async (id: number, fieldsToUpdate: Partial<{ 
    ...
    const query = `UPDATE postimage SET ${setClause} WHERE id = ?`;
...
export const deletePostImageByPostId = async (postid: number): Promise<any> => {
    const [results] = await db.query('DELETE FROM postimage WHERE postid = ?', [postid]);
    return {
        success: true,
